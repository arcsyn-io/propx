...
func TestBankAccount(t *testing.T) {
	sm := propx.StateMachine[BankAccount, BankCommand]{
		...
		Commands: []propx.Command[BankAccount, BankCommand]{
			{
				Name: "deposit",
				Generator: propx.Map(propx.IntRange(1, 1000), func(amount int) BankCommand {
			...
			{
				Name:      "close",
				Generator: propx.Const(BankCommand{Type: "close", Amount: 0}),
				Execute: func(state BankAccount, cmd BankCommand) (BankAccount, error) {
	...
	propx.TestStateMachine(t, sm, cfg)
}
...
func TestCounter(t *testing.T) {
	sm := propx.StateMachine[Counter, CounterCommand]{
		InitialState: Counter{Value: 0},
		Commands: []propx.Command[Counter, CounterCommand]{
			{
				Name: "increment",
				Generator: propx.Map(propx.IntRange(1, 10), func(delta int) CounterCommand {
			...
			{
				Name:      "reset",
				Generator: propx.Const(CounterCommand{Type: "reset", Delta: 0}),
				Execute: func(state Counter, cmd CounterCommand) (Counter, error) {
...
func TestCache(t *testing.T) {
	sm := propx.StateMachine[Cache, CacheCommand]{
		InitialState: Cache{Data: make(map[string]string), Size: 0, MaxSize: 100},
		Commands: []propx.Command[Cache, CacheCommand]{
			{
				Name: "set",
				Generator: propx.Bind(
					propx.StringAlphaNum(propx.Size{Min: 1, Max: 10}),
					func(key string) propx.Generator[CacheCommand] {
						return propx.Map(
							propx.StringAlphaNum(propx.Size{Min: 1, Max: 20}),
			...
			{
				Name: "get",
				Generator: propx.Map(propx.StringAlphaNum(propx.Size{Min: 1, Max: 10}), func(key string) CacheCommand {
			...
			{
				Name:      "clear",
				Generator: propx.Const(CacheCommand{Type: "clear", Key: "", Value: ""}),
				Execute: func(state Cache, cmd CacheCommand) (Cache, error) {
